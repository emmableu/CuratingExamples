...
label_color_dict = {'one_hot':['#CC4F1B', '#FF9848'],
                  'pq_gram': ['#1B2ACC', '#089FFF'],
                  'one_hot_dpm':['#3F7F4C', '#7EFF99'],
                  'pq_gram_dpm':['#E958A1', '#EB8ABA']}
...
label_s = [ 'one_hot_dpm',  'pq_gram_dpm']
...
for action_index, action_name in enumerate(action_name_s):
    ...
    for label in label_s:
        data = load_obj(label_file_dict[label], base_dir+"Simulation/PatternMining/SessionTable/", action_name)
