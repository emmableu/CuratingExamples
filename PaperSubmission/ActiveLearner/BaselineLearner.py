...
def save_performance_for_one_repetition(new_row, save_dir, code_shape_p_q_list, repetition):
...
def recall_curve(label_name, code_shape_p_q_list):
    ...
    for repetition in range(10):
        ...
        save_dir = base_dir + "/Simulation/PatternMining/SessionTable/ActiveLearning/RecallCurve/" + label_name
        if repetition == 0:
            atomic_save_hit_for_one_repetition(new_row, save_dir, code_shape_p_q_list, repetition)
        else:
            atomic_save_hit_for_one_repetition(new_row, save_dir, code_shape_p_q_list, repetition)
...
def encapsulated_simulate():
    ...
    code_shape_p_q_list1 = [[1, 0], [1, 1], [1, 2], [1, 3], [2, 3]]
    code_shape_p_q_list2 = [[1, 0]]
    for code_shape_p_q_list in [code_shape_p_q_list2, code_shape_p_q_list1]:
