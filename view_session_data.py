...
# label_name_s = ['cochangescore', 'jump',   'movetomouse', 'moveanimate', 'costopall']
# label_name_s = ["keymove", 'jump', 'cochangescore']
label_name_s = ['keymove', 'cochangescore', 'jump',   'movetomouse', 'moveanimate']
...
def get_all_data():
    ...
    for label_index, label_name in enumerate(label_name_s):
        ...
        data = load_obj('all_simulation_' + label_name + "[1, 0]baseline",
                                    base_dir, 'simulation/no_model_selection')
        all_simulation_1 = data
        # data = load_obj('all_simulation_' + label_name,
        #                             base_dir, 'simulation/model_feature_no_voi_step=5/subtrain/session0')
        #


        data = load_obj('all_simulation_' + label_name,
                                    base_dir, 'simulation/model_feature_no_voi_step=5/add_random_to_train/')
        # all_simulation_1 = {0: data}

        # data = load_obj('all_simulation_' + label_name,
        #                             base_dir, 'simulation/model_feature_voi_selection/subtrain/session4')
        # print(data)
        all_simulation_2 = data
        # all_simulation_3 = load_obj('all_simulation_' + label_name + "code_state[[1, 0], [1, 1], [1, 2], [1, 3]]",
        #                             base_dir, 'simulation')
        for s_i, all_simulation in enumerate([all_simulation_1,  all_simulation_2]):
        # for all_simulation in [all_simulation_1, all_simulation_2]:
            ...
            game_y = load_obj('y_train', orig_dir, "")
            ...
            for number_index, number in enumerate(x_axis):
                ...
                baseline_y = create_baseline(start_data, total_pos, total, len(x_axis), step, 0.5)
                this_sum = 0
                for iteration_item in range(all_repetitions):
            ...
            if s_i == 0:
                half_y = [y/2 for y in average_y]
                all_data[label_name].append([x_axis, baseline_y, half_y, best_y, auc, total, total_pos])
            else:
...
def plot_all():
    ...
    for label_index, label_name in enumerate(label_name_s):
        ...
        # print(all_data[label_name][min_index][0], all_data[label_name][min_index][1])

        plt.plot(all_data[label_name][min_index][0], all_data[label_name][min_index][1], marker='o',
                 markerfacecolor='red', markersize=1,
                 color='red', linewidth=2)
        plt.plot(all_data[label_name][min_index][0], all_data[label_name][min_index][3], marker='o',
                 markerfacecolor='red', markersize=1,
                 color='red', linewidth=2)
        plt.plot(all_data[label_name][min_index][0], all_data[label_name][0][2][:min_len], marker='o',
                 markerfacecolor='black', markersize=1,
                 color='black', linewidth=2, label='baseline')
        plt.plot(all_data[label_name][min_index][0], all_data[label_name][1][2][:min_len], marker='o',
                 markerfacecolor='blue', markersize=1,
                 color='blue', linewidth=2, label='model selection')
        # plt.plot(all_data[label_name][min_index][0], all_data[label_name][2][2][:min_len], marker='o',
        #          markerfacecolor='blue', markersize=1,
        #          color='purple', linewidth=2, label='best feature & model')
        plt.gca().set_yticklabels(
            ['{:.0f}%'.format(x * 100 / all_data[label_name][0][6]) for x in plt.gca().get_yticks()])
        plt.gca().set_xticklabels(
            ['{:.0f}%'.format((x + start_data + 1) * 100 / all_data[label_name][0][5]) for x in
             plt.gca().get_xticks()])
        # ax.set_title(label_name_dict[label_name] + " #P =" + str(all_data[label_name][0][6]) + " AUC = " + str(
        #     round(all_data[label_name][2][4], 2)) + "vs" + str(round(all_data[label_name][1][4], 2)) + "vs" + str(
        #     round(all_data[label_name][0][4], 2)))
